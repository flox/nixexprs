{ lib, utils, floxPath, floxFile, importingChannel, ownChannel, trace
, floxPathDepth, baseScope, channels, flox, superScope, getChannelSource }:
let
  ...
  meta = {
    ...
    withVerbosity = throw
      "meta.withVerbosity as used in ${floxFile} was removed, use `meta.trace <subsystem> <verbosity> <message> <value>` instead";
    inherit trace channels;

    mapDirectory = dir:
      trace.withContext "mapDirectory" (baseNameOf dir) (trace:
        { call ? path: utils.callPackageWith trace scope path }:
        lib.mapAttrs (name: value: call value.path) (utils.dirToAttrs trace dir)
        // {
          recurseForDerivations = true;
        });

    importNix = { channel ? importingChannel, project, path, ... }@args:
      trace.withContext "importNix" "" (trace:
        let
