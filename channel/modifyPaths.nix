      ...
      go = index: list: original:
        let
          ...
          withNestedMods =
            ...
            if lib.length split.wrong == 0 then original
            ...
            # set because a modification that requires recursing into one was provided
              ...
              lib.mapAttrs (name: go (index + 1) (grouped.${name} or [])) original;
...
in {
  ...
  overlayExample = import <nixpkgs> {
    overlays = [(self: modifyPaths [
      {
      ...
      }
      {
        ...
        mod = x: x.override (old: {
          overrides = lib.composeExtensions old.overrides (pself: psuper: {
            toml = throw "nope";
          });
        });
      }
