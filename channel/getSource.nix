...
channel:
trace.withContext "channel" channel (trace:
  let
  ...
  in project:
  trace.withContext "project" project (trace:
    let
    ...
    let
      ...
      components = let
        ...
        original = if channelSourceOverrides ? ${project} then
          trace "getSource" 5 "Using overridden source"
          channelOverrideComponents
        else if overrides ? src then
          trace "getSource" 5 "Using src argument in package definition"
          argumentOverrideComponents
        else
