{ firstArgs, channelName, lib, utils, trace, packageSets }:
let
  ...
  dependencies = let
    ...
    fileDeps = if builtins.pathExists dependencyFile then
      lib.importJSON dependencyFile
    else
      [ ];
  ...
  packageSpecs = lib.mapAttrs (setName: _:
    utils.dirToAttrs (trace.setContext "dir" "${channelName}/${setName}")
    (topdir + "/${setName}")) packageSets;

  rootFile = topdir + "/default.nix";

in { inherit dependencies packageSpecs conflictResolution rootFile; }
